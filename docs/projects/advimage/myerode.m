function newimg=myerode(img,struct)
%Todd Goldfinger
%erodes a binary image (1s and 0s only) using a 3x3 structuring element
   ...
   if (r(i)~=s(1)+1) & (r(i)~=2) & (c(i)~=s(2)+1) & (c(i)~=2)
      %if the pixel before and after a row, or column are different, we're on the border 
		if (tmpimg(r(i)-1,c(i)) ~= tmpimg(r(i)+1,c(i)) | (tmpimg(r(i),c(i)-1) ~= tmpimg(r(i),c(i)+1)))
      	newimg(r(i)-1:r(i)+1,c(i)-1:c(i)+1)=structprime.*tmpimg(r(i)-1:r(i)+1,c(i)-1:c(i)+1);
   	end
	end
end
%fix the image size
